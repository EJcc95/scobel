...
description: >-
  Use when generating SEO pages at scale from data: location pages, "X vs Y"
  comparison pages, "alternatives to X", integration pages, feature templates,
  or any pattern producing hundreds of pages from rows. Not for one-off
  marketing pages or thin/spammy content.
---
...
create index location_pages_published_idx on location_pages (published) where published = true;
```
...
- One canonical URL per row — never multiple server-side authorization leading to the same content.
...
- **Build-time**: render via Satori → store in S3-compatible object storage → reference in `og:image`.
