...
- MUST: Treat Node.js + Express as the backend and MySQL as the primary database.
- MUST: Keep database access in backend repositories/services; frontend code uses typed HTTP API clients.
- MUST: Use Better Auth for sessions, cookies, authentication, and route guards.
- MUST: Enforce ownership, organization membership, and roles in Express after session validation.
- NEVER: Put database credentials, Better Auth secrets, or privileged API keys in frontend variables (`VITE_*`, `PUBLIC_*`).
- SHOULD: Use TanStack Query for server state; use Zod on both frontend forms and Express request boundaries.
- SHOULD: Treat PostgreSQL-specific skills (`postgres-*`, `pg-*`) as opt-in only; do not use them for MySQL projects.
- SHOULD: Use `node-express-saas-architecture` when creating or reorganizing backend structure.

## Shared tooling

- MUST: Use `pnpm` exclusively for installing dependencies, running scripts, and managing workspaces.
- MUST: Accept `pn` as the short form only when it is configured as an alias for `pnpm`.
- NEVER: Use `npm`, `yarn`, or `bun` commands for this project.
- MUST: Do not mix lockfiles; keep only the repository's `pnpm-lock.yaml`.

## Frontend work

- MUST: Use the existing frontend stack and keep pages/components focused and composable.
- MUST: Consume backend data through typed HTTP clients and TanStack Query/SWR hooks.
...
- MUST: Use Node.js + Express with the modular structure from `node-express-saas-architecture`.
- MUST: Keep the flow `route → controller → service → repository → MySQL`.
- MUST: Use Better Auth for sessions and derive authorization from the authenticated session.
- MUST: Validate bodies, params, and query strings with Zod before service execution.
- MUST: Store logs/runtime files outside `src`; keep `logs/`, `uploads/`, and `tmp/` ignored.
- MUST: Use MySQL-compatible SQL and versioned migrations; PostgreSQL/`pg-*` patterns are opt-in only.
- NEVER: Trust `user_id`, `organization_id`, roles, or payment status supplied by the client.
